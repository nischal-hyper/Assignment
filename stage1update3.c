...
const char *day_names[DAYS_IN_WEEK] = {
...
char member_routine[MAX_MEMBERS][DAYS_IN_WEEK][MAX_ROUTINE_LENGTH];
...
void setExerciseRoutine() {
    ...
    listMembers();
    for (int i = 0; i < member_count; i++) {
        printf("\nEnter routine for %s (%s):\n", milliways_members[i].name, milliways_members[i].nickname);
        int core_used = 0;
        for (int d = 0; d < DAYS_IN_WEEK; d++) {
            char cardio[30], strength[30];
            printf("%s - Choose one cardio exercise (Running, Cycling, Fast Walking): ", day_names[d]);
            fgets(cardio, sizeof(cardio), stdin);
            cardio[strcspn(cardio, "\n")] = 0;

            do {
                printf("%s - Choose one strength/core exercise (Push-ups, Squats, Leg Press, Leg Curl, Pull-ups, Chin-ups, Plank, Crunches): ", day_names[d]);
                fgets(strength, sizeof(strength), stdin);
                strength[strcspn(strength, "\n")] = 0;
                if ((strcmp(strength, "Plank") == 0 || strcmp(strength, "Crunches") == 0) && core_used >= 1) {
                    printf("Core exercise already used once this week. Choose another.\n");
                } else break;
            } while (1);
            ...
            snprintf(member_routine[i][d], MAX_ROUTINE_LENGTH, "%s, %s", cardio, strength);
        }
    }
    printf("Workout routines recorded.\n\n");
}
...
void getExerciseRoutine() {
    ...
    for (int i = 0; i < member_count; i++) {
        if (strcmp(name, milliways_members[i].name) == 0) {
            ...
            for (int d = 0; d < DAYS_IN_WEEK; d++) {
                printf("%s: %s\n", day_names[d], member_routine[i][d]);
            }
            return;
        }
    }
